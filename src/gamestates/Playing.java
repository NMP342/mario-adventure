...
public class Playing extends State implements Statemethods {
    ...
    public void update() {
        if (paused) {
            ...
            updateDialogue();
            if (drawRain) {
                rain.update(xLvlOffset, yLvlOffset);
            }
            levelManager.update();
...
//        yLvlOffset
    ...
    public void draw(Graphics g) {
        ...
        g.drawImage(backgroundImg, 0, 0 - yLvlOffset, Game.GAME_WIDTH, 2 * Game.GAME_HEIGHT, null);
//        g.drawImage(backgroundImg, 0, 0 - yLvlOffset, Game.GAME_WIDTH, (int) (backgroundImg.getHeight() * Game.SCALE), null);
        ...
        if (drawShip && levelManager.getLevelIndex() == 0) {
            g.drawImage(shipImgs[shipAni], (int) (100 * Game.SCALE) - xLvlOffset, (int) ((288 * Game.SCALE) + shipHeightDelta) - yLvlOffset, (int) (78 * Game.SCALE), (int) (72 * Game.SCALE), null);
        }
    ...
    private void drawClouds(Graphics g) {
        for (int i = 0; i < 4; i++) {
            g.drawImage(bigCloud, i * BIG_CLOUD_WIDTH - (int) (xLvlOffset * 0.3), (int) (204 * Game.SCALE - yLvlOffset), BIG_CLOUD_WIDTH, BIG_CLOUD_HEIGHT, null);
        }

        for (int i = 0; i < smallCloudsPos.length; i++) {
            g.drawImage(smallCloud, SMALL_CLOUD_WIDTH * 4 * i - (int) (xLvlOffset * 0.7), smallCloudsPos[i] - yLvlOffset, SMALL_CLOUD_WIDTH, SMALL_CLOUD_HEIGHT, null);
        }
    }
        ...
        if (!gameOver && !gameCompleted && !lvlCompleted) {
            switch (e.getKeyCode()) {
                ...
                case KeyEvent.VK_ESCAPE:
                    ...
                    break;
//                case KeyEvent.VK_B:
//                    player.usePotion(player.getBluePotionNumber(), BLUE_POTION);
//                    break;
                case KeyEvent.VK_R:
                    player.usePotion(player.getRedPotionNumber(), RED_POTION);
                    break;
            }
        }
    }
    ...
    public void setLevelCompleted(boolean levelCompleted) throws IOException {
    ...
    public int getRedPotionNumber() {
        return player.getRedPotionNumber();
    }

    public int getGreenPotionNumber() {
        return player.getGreenPotionNumber();
    }

    public void changeRedPotionNumber(int number) {
        player.changeRedPotionNumber(number);
    }

    public void changeGreenPotionNumber(int number) {
        player.changeGreenPotionNumber(number);
    }
    ...
    public boolean isDoubleHealth() {
        return player.isDoubleHealth();
    }

    public boolean isBuyGreenPotion() {
        return player.isBuyGreenPotion();
    }
